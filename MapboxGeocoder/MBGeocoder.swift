...
let defaultAccessToken = Bundle.main.infoDictionary?["MGLVietMapsAccessToken"] as? String
...
open class Geocoder: NSObject {
    ...
    public typealias JsonCompletionHandler = (_ jsonResponse: [String:Any]?, _ error: NSError?) -> Void
    ...
    @objc public init(accessToken: String?, host: String?) {
        ...
        baseURLComponents.host = host ?? "api.vietmaps.vn"
    ...
    open func geocode(_ options: GeocodeOptions, completionHandler: @escaping CompletionHandler) -> URLSessionDataTask {
        ...
        let task = dataTaskWithURL(url, completionHandler: { (data) in
            ...
            do {
                ...
                completionHandler(result.placemarks, "VietMaps", nil)
            } catch {
    ...
    @discardableResult
    @objc open func getInfoLocation(_ options: ReverseGeocodeOptions, completionHandler: @escaping JsonCompletionHandler) -> URLSessionDataTask {
        let url = urlForGeocoding(options)

        let task = dataTaskWithURL(url, reverseGeocoder: false, completionHandler: { (data) in
            guard let data = data else { return }
            do {
    ...
    open func batchGeocode(_ options: GeocodeOptions & BatchGeocodeOptions, completionHandler: @escaping BatchCompletionHandler) -> URLSessionDataTask {
        ...
        let task = dataTaskWithURL(url, completionHandler: { (data) in
            ...
            do {
                ...
                let attributionsByQuery = result.map { _ in return "VietMaps" }
     ...
     */
    fileprivate func dataTaskWithURL(_ url: URL, reverseGeocoder: Bool = false, completionHandler: @escaping (_ data: Data?) -> Void, errorHandler: @escaping (_ error: NSError) -> Void) -> URLSessionDataTask {
        var request = URLRequest(url: url)
        let authenHeader = "Bearer " + accessToken;
        request.setValue(userAgent, forHTTPHeaderField: "User-Agent")
        request.setValue(authenHeader, forHTTPHeaderField: "Authorization")
    ...
    @objc open func urlForGeocoding(_ options: GeocodeOptions) -> URL {
        ...
        var unparameterizedURL = URL(string: "/search/geocoders/q/" + queryComponent, relativeTo: apiEndpoint)!
        if type(of: options) == ReverseGeocodeOptions.self {
            let reverseOptions = options as! ReverseGeocodeOptions
            if reverseOptions.coordinates.count > 0 {
                var urlStr = "/search/reverse-geocoders/?format=json&lat=" + String(reverseOptions.coordinates.first!.latitude)
                urlStr += "&lon=" + String(reverseOptions.coordinates.first!.longitude)
                unparameterizedURL = URL(string: urlStr, relativeTo: apiEndpoint)!
            }
        }
